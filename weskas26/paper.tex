...
\begin{figure}
	...
	\begin{subfigure}[t]{0.32\textwidth}
		\centering
		\raisebox{1em}{\includegraphics[width=.7\textwidth]{pdf/2d-block}}
		\caption{A 2D block distribution resulting in memory-optimal multiplication.}
		\label{fig:2d-block}
	\end{subfigure}
	\caption{Cube graphs and their application to matrix--matrix multiplication.}
\end{figure}
...
However, the result is not an immortal algorithm per se:
while the algorithm is communication- and work-optimal, its processor-local memory requirement is proportional to $n^2/p^{2/3}$,
which is larger than the ideal bound of $n^2/p$.
By subdividing the matrices $A$, $B^T$, and $C$ using a standard 2D block distribution depicted in Figure~\ref{fig:2d-block},
a work- and memory-optimal algorithm with costs proportional to $n^3/p$ and $n^2/p$, respectively, may be constructed.
Indeed such algorithms are not communication-optimal, however, as processors need to communicate $\sqrt{P}-1$ blocks of size $n^2/p$,
for a total communication cost proportional to $n^2/p^{1/2}$, which is higher than $n^2/p^{2/3}$.
In fact, there is a discrete trade-off space between communication- and memory-optimality,
resulting into an immortal algorithm that exposes an explicit trade-off parameter~\cite{mccoll99}.
An algorithm that adapts to run-time conditions such as system parameters $p$, $g$, $l$, or available memory, is known as a \emph{poly-algorithm}.
